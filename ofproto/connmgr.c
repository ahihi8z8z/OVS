...
void send_rst_packet(struct dp_packet packet_in)
{
    // open log file
    FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
    fprintf(log,"Entry send_rst_packet() function\n.......................................................\n"); 
    ...
    // else
    // {
    //     fprintf(log, "RST packet Sent with ack_seq : %u - htonl(%u) - ntohl(%u) \n.......................................................\n", tcph->ack_seq, htonl(tcph->ack_seq), ntohl(tcph->ack_seq));
    // }
    fclose(log);
    free(datagram);
    close(server_socket);
                            ...
                            const struct ofproto_async_msg *am)
{
    ...
    // xu ly goi ACK
    else if (htons(tflow.tcp_flags) == 16 && (tflow.nw_src == uncheck_syn_flow.nw_src) && (tflow.nw_dst == uncheck_syn_flow.nw_dst) && (tflow.tp_src == uncheck_syn_flow.tp_src) && (tflow.tp_dst == uncheck_syn_flow.tp_dst))
    {
        // char *buf = malloc(300);
        // FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
    ...
    // xu ly goi SYN/ACK
    else if (htons(tflow.tcp_flags) == 18 && (tflow.nw_src == uncheck_syn_flow.nw_dst) && (tflow.nw_dst == uncheck_syn_flow.nw_src) && (tflow.tp_src == uncheck_syn_flow.tp_dst) && (tflow.tp_dst == uncheck_syn_flow.tp_src))
    {
        FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
        ...
        // else if (pid == 0) {
        //     // Child process
        // send_ack_syn_packet(*tpacket); // Calling function in child process
        //     exit(EXIT_SUCCESS);
        // }
        // else {
        //     // Parent process
        ...
        int systemRet1 = system(bar1);
        log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
        fprintf(log,"Status when add flow 1 %d \n",systemRet1);
        fclose(log);

        int systemRet2 = system(bar2);
        ...
        send_rst_packet(uncheck_syn_packet);

        check = false;
    }

    // xu ly goi PSH/ACK
    else if (htons(tflow.tcp_flags) == 24 && (tflow.nw_src == uncheck_syn_flow.nw_src) && (tflow.nw_dst == uncheck_syn_flow.nw_dst) && (tflow.tp_src == uncheck_syn_flow.tp_src) && (tflow.tp_dst == uncheck_syn_flow.tp_dst))
    {
        FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
